...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//psuedocode:
	//select MAX3421E
	XSpi_SetSlaveSelect(&SpiInstance, SpiInstance.SlaveSelectMask);  // XSpi SetSlaveSelectReg(InstancePtr, Mask) https://xilinx.github.io/embeddedsw.github.io/spi/doc/html/api/group__spi.html#ga162523a3e9b29f063701db303ac8cf17
	//write reg + 2 via SPI
	...
	status = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, buffer_size);  // int Spi_Trasfer(XSpi *InstancePtr, u8 *SendBufPtr, u8 *RecvBufPtr, unsigned int ByteCount)  https://xilinx.github.io/embeddedsw.github.io/spi/doc/html/api/group__spi.html#ga4c44c082ef2d2b9cf4ba3db8bcebd954
	//read return code from SPI peripheral (see Xilinx examples)
	// IDK HOW TO DO THIS
	//if return code != 0 print an error

	if (status != 0) {
		xil_printf("Error writing register: %x\n", status);
	}
...
/* multiple-byte write */
/* returns a pointer to a mem=ory position after last written */
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	int buffer_size = nbytes + 1;
	int status;
	...
	//write reg + 2 via SPI
	status = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, buffer_size);
	if (status != 0) {
			xil_printf("Error writing register: %x\n", status);
	}
...
//	for (BYTE n = 0; n < nbytes; n++) {
////		XSpi_Transfer(&SpiInstance, &data[n], NULL, 1);
//		WriteBuffer[n+1] = data[n];
//		status = XSpi_Transfer(&SpiInstance, WriteBuffer, NULL, buffer_size);
//		if (status != 0) {
//				xil_printf("Error writing register: %x\n", status);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//read return code from SPI peripheral
//	XSpi_Transfer(&SpiInstance, NULL, &status, 1);
//	//if return code != 0 print an error
//	if (status != 0) {
//		xil_printf("Error reading register: %x\n", status);
//	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	return (data + nbytes);
