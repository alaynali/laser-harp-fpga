...
begin:Color_on_proc
	...
	if (DrawY >= 6*DrawX-996 && DrawY <= 6*DrawX-960 && DrawY <= 355 && DrawY >= 10)
		...
		yellow_on = 1'b0;
	// green
	if (DrawX >= 241 && DrawX <= 246 && DrawY <= 355 && DrawY >= 10)
		green_on = 1'b1;
	else
		green_on = 1'b0;
	// blue
...
begin:Carrot_on_proc
	// red
	if ((DrawX-179)*(DrawX-179) + (DrawY-353)*(DrawY-353) <= 20)
		RedCarrot_on = 1'b1;
	else
		RedCarrot_on = 1'b0;
	// orange
	if ((DrawX-201)*(DrawX-201) + (DrawY-353)*(DrawY-353) <= 20) 
		OrangeCarrot_on = 1'b1;
	else 
		OrangeCarrot_on = 1'b0;
	// yellow
	if ((DrawX-222)*(DrawX-222) + (DrawY-353)*(DrawY-353) <= 20)
		YellowCarrot_on = 1'b1;
	else
		YellowCarrot_on = 1'b0;
	// green
	if ((DrawX-243)*(DrawX-243) + (DrawY-353)*(DrawY-353) <= 20)
		GreenCarrot_on = 1'b1;
	else
		GreenCarrot_on = 1'b0;
	// blue
	if ((DrawX-264)*(DrawX-264) + (DrawY-353)*(DrawY-353) <= 20)
		BlueCarrot_on = 1'b1;
	else
		BlueCarrot_on = 1'b0;
	// indigo
	if ((DrawX-285)*(DrawX-285) + (DrawY-353)*(DrawY-353) <= 20)
		IndigoCarrot_on = 1'b1;
	else
		IndigoCarrot_on = 1'b0;
	// violet
	if ((DrawX-307)*(DrawX-307) + (DrawY-353)*(DrawY-353) <= 20) 
...
begin:Red_int_proc
	...
	// 		RedY_next = 9'd10;
	// 	end
	// 	if (JAB_0 || (l_click && CursorY >= 2*CursorX-16 && CursorY <= 2*CursorX+4 && CursorY <= 355 && CursorY >= 11)) begin
	// 		red_click_next = 1'b1;
	// 		RedY_next = CursorY;
	// 	end
	// end
...
begin:Orange_int_proc
	...
	// 	end
	// end
	// else if (!JAB_1) begin // doesn't work
	// 	orange_click_next = 1'b0;
	// 	OrangeY_next = 9'd10;
	// end
	// else begin
...
begin:Violet_int_proc
	violet_int = 1'b0;
	violet_click_next = violet_click;
	VioletY_next = VioletY;

	if (l_click || r_click) begin
	   if (CursorY >= 347 && CursorY <= 357 && CursorX >= 302 && CursorX <= 312 || (r_click && CursorY >= -2*CursorX+956 && CursorY <= -2*CursorX+976 && CursorY <= 355 && CursorY >= 11)) begin
